...
while gameover is False:
    # dfunction to choose a random dice to roll
    dice_function = [rollD100, rollD4, rollD12, ]
    dice_choice = random.choice(dice_function) 
    dice_choice2 = random.choice(dice_function)
    print(f"this is round {rounds}") 
    print (f"you get to roll a {dice_choice}, and a {dice_choice2} this round")
    ...
    for i in range(number_of_players): 
      roll = dice_choice()
      roll2 = dice_choice2()
      total_roll = roll +  roll2 # get the total of the rolls
      total_roll_list[i].append(total_roll) # a list to store the total amount of both rolls
      current_rolls.append((roll, roll2))
      player_rolls_history[i].append((roll, roll2))
      print(f"{player_names[i]} rolled: {roll}, and a {roll2} for a total of {total_roll}")

  # enter to keep going ?
    input("\nPress ENTER to continue...")

  # determine the highest roll
    max_roll = max(total_roll_list)
    ...
    if gameover is False:
      rounds += 1
      print(f"the battle continues to round {rounds}!")
...
with open(filename, 'w') as file:
  for rounds_played in range(rounds):
    ...
    for i in range(number_of_players):
      rolls_str (f"{player_names[i]} rolled {player_rolls_history[i][rounds_played]} with a total of {total_roll_list}")
     # if i < number_of_players - 1:  # to add a comma between rolls except for the last one
       # rolls_str += ", "
